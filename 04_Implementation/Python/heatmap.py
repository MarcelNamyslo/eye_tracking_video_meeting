...
"""
Donutchart Class, for generating the heatmap seen in the final statistic

We used the code from the Gaze Point Heat Map repository of Tobias Röddiger and adjusted it for our case
This repo can be found at  https://github.com/TobiasRoeddiger/GazePointHeatMap
"""
...
def draw_heatmap(gazepoints, dispsize, imagefile=None, alpha=0.5, savefilename=None, gaussianwh=100, gaussiansd=None):
    ...
    heatmap = numpy.zeros(
        (int(heatmapsize[0]), int(heatmapsize[1])), dtype=float)
    # create heatmap
        ...
        if (not 0 < x < dispsize[0]) or (not 0 < y < dispsize[1]):
            ...
            try:
                heatmap[y:y + vadj[1], x:x + hadj[1]] += gaus[vadj[0]
                    :vadj[1], hadj[0]:hadj[1]] * gazepoints[i][2]
            except:
    ...
    # resize heatmap
    heatmap = heatmap[int(strt):int(dispsize[1] + strt),
                      int(strt):int(dispsize[0] + strt)]
    # remove zeros
